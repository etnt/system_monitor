...
-spec handle_continue(Continue :: term(), State :: term()) ->
          {noreply, State :: term()} |
          {noreply, State :: term(), Timeout :: erlang:timeout()} |
          {noreply, State :: term(), hibernate} |
          {noreply, State :: term(), {continue, Continue :: term()}} |
          {stop, Reason :: normal | term(), State :: term()}.
...
handle_info({gun_upgrade, ConnPid, StreamRef, [<<"websocket">>], _Headers},
    ...
    {noreply, State#{conn_pid   => ConnPid,
                     stream_ref => StreamRef}};
%%
handle_info({gun_ws, _ConnPid, _StreamRef, _Msg}, State) ->
    ?dbg("got msg: ~p~n",[_Msg]),
    {noreply, State};
%%
handle_info({gun_down, ConnPid, ws, closed, StreamRef},
            #{conn_pid   := ConnPid,
              stream_ref := StreamRef} = State) ->
    ?dbg("handle_info got closed connection~n",[]),
